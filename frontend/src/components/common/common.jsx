...
export function StatusBadge({ status }) {
  ...
  return (
    <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-[11.5px] font-semibold whitespace-nowrap ${style}`}>
...
export function EmptyState({ icon: Icon, title, description, action }) {
  return (
    <div className="flex flex-col items-center justify-center py-16 px-6 text-center gap-3">
      ...
      <div className="text-[15px] font-semibold text-admin-secondary">{title}</div>
      {description && (
...
export function StatCard({ icon: Icon, label, value, badge, badgeColor }) {
  return (
    <div className="bg-white border border-admin-border rounded-xl p-5 shadow-sm hover:shadow-md hover:-translate-y-px transition-all duration-200">
      ...
      {badge && (
        <div className={`mt-1.5 text-[12px] font-medium ${badgeColor || 'text-status-success'}`}>
          {badge}
        </div>
      )}
    </div>
        ...
        <div className="flex flex-col items-center text-center mb-5">
          <div
            className={`w-14 h-14 rounded-full flex items-center justify-center mb-3.5 ${
              danger ? 'bg-status-danger-bg text-status-danger' : 'bg-primary-bg text-primary'
            }`}
          >
...
export function Pagination({ currentPage, totalPages, onPageChange }) {
  ...
  const btnDefault  = 'border-admin-border bg-transparent text-admin-secondary hover:bg-surface-2 hover:border-primary hover:text-primary';
  ...
  return (
    <div className="flex items-center justify-end gap-1 px-4 py-3.5 border-t border-admin-border-light">
      ...
      {start > 1 && (
        <>
          <button className={btnBase + btnDefault} onClick={() => onPageChange(1)}>1</button>
      ...
      {end < totalPages && (
        <>
          {end < totalPages - 1 && <span className="px-1 text-admin-muted">…</span>}
          <button className={btnBase + btnDefault} onClick={() => onPageChange(totalPages)}>{totalPages}</button>
        </>
      )}
